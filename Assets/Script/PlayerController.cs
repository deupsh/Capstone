...
public class PlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (rb == null)
        {
            Debug.LogError("Rigidbody2D가 Player 컴포넌트에 없습니다.");
        }
    ...
    void Update()
    {
        ...
        moveDirection = new Vector2(moveX, moveY).normalized; // 방향 벡터 정규화
    }
    ...
    private void AdjustSpeedBasedOnTile()
    {
        ...
        if (currentTile != null)
        {
            ...
            {
                moveSpeed = playerStats.SPEED - 1f; // 이동 속도 감소
            }
            else
            {
                moveSpeed = playerStats.SPEED; // 기본 속도로 설정
            }
        }
    }
    ...
    private void LoadPlayerData()
    {
        ...
        if (File.Exists(path))
        {
            ...
            {
                Debug.LogError("플레이어 데이터가 존재하지 않습니다.");
            }
        }
        else
        {
